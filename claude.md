...
- **`reassign_matches_on_profile_delete()`** — `SECURITY DEFINER` trigger function (`SET search_path = 'public'`) fired by the `before_profile_delete` trigger on `profiles`. The sole mechanism for reassigning match FK columns (`winnerId`, `loserId`, `reporterId`, `confirmedById`, `rejectedById`) from a deleted user to the `[deleted]` sentinel UUID. Fires automatically during GoTrue's CASCADE delete of `auth.users` → `profiles`. The `search_path` setting is critical: the GoTrue execution context uses an `auth`-only `search_path` — without `SET search_path = 'public'`, the `"Matches"` table reference fails.
