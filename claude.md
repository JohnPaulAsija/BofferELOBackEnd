...
- `DELETE /users/me` — delete own account; match history preserved via sentinel reassignment; returns `{"deleted": "<user_id>"}` (requires `Authorization` header)
...
- **`delete_user_profile(p_user_id)`** — `SECURITY DEFINER` function that reassigns all match FK columns (`winnerId`, `loserId`, `reporterId`, `confirmedById`, `rejectedById`) referencing the given user to the `[deleted]` sentinel UUID, then deletes the profile row. Currently unused by the app (match reassignment is done in Python via `_reassign_matches_to_sentinel` due to PostgREST schema cache latency) but available as a DB-level fallback.
